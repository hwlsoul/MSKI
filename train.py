...
#     wandb.init(project="DocRED")
    ...
    distant_file = os.path.join(args.data_dir, args.distan_file)
    rel_file = os.path.join(args.data_dir, args.distan_file)
    ...
    #     data_rel = json.load(fh)
    # rel_set = [' '.join(words) for words in data_rel['relation']]
    
    # rels_input = tokenizer(rel_set, padding=True, truncation=True, return_tensors='pt').to(0)
    ...
    #train teacher model in distant dataset        
        train(args, model, distant_features, dev_features, test_features)
